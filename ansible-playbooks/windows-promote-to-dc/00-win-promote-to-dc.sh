...
init_vars() {
	...
	HOSTVARS_PATH="./roles/win-promote-to-dc/vars/hostvars"
...
execute_single() {
	ansible-playbook win-promote-to-dc.yml -i "$INI_FILE" -e \
...
create_files_list() {
	...
	mode=$7
	safemodepass=$8
	yml_filename="./roles/win-promote-to-dc/vars/hostvars/" hostname ".yml"
	printf "domain: %s\nnetbios: %s\nmode: %s\nsafemodepass: %s", \
...
execute_list() {
	ansible-playbook win-promote-to-dc.yml -i "$INI_FILE" -e "load_hostvars=true"
}
